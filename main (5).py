...
async def test_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    price_ton = fake_purchase["price_nanoton"] / 1_000_000_000
    time_str = datetime.fromtimestamp(fake_purchase["timestamp"], tz=timezone.utc).strftime("%d/%m/%Y %H:%M UTC")
    
    nft_addr = fake_purchase["nft_address"]
    buyer_addr = fake_purchase["buyer"]
    ...
    message = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🏷️ *NFT:* [Precious Peach #42]({nft_link})\n"
        f"💰 *Price:* {price_ton:.4f} TON\n"
        f"🛒 *Buyer:* [{shorten(buyer_addr)}]({buyer_link})\n"
        f"🕐 *Time:* {time_str}\n"
        f"━━━━━━━━━━━━━━━━━━━━"
    ...
    log.info(f"/test executed by {update.effective_user.first_name}")
...
# ─── POLLING LOOP ──────────────────────────────────────────────────────────
async def polling_loop(bot: Bot, group_id: int):
...
async def main():
    ...
    await polling_loop(bot, TELEGRAM_GROUP_ID)
