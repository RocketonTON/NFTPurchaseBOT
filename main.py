...
async def check_commands(bot: Bot):
    ...
    try:
        ...
        log.debug(f"🔄 Command check: Last update ID: {last_id}")
        ...
        for update in updates:
            ...
            log.debug(f"📩 Message in {chat_type} chat {chat_id}: {text[:50]}...")
            ...
            if "/start" in text.lower() or "/help" in text.lower():
                log.debug("📝 Processing /start or /help command")
                await bot.send_message(
                    chat_id=chat_id,
                    text=(
                ...
                log.debug("📝 Processing /test command")
                test_msg = (
                    f"🍑 *Test Notification*\n"
                    f"━━━━━━━━━━━━━━━━━━━━\n"
                    f"✅ Bot is working!\n"
            ...
            if update.update_id > last_id:
                ...
                log.debug(f"💾 Saved new last update ID: {last_id}")
...
async def main():
    ...
    try:
        await asyncio.gather(
            nft_polling_loop(bot, target_group_id),
            command_check_loop(bot)
        )
